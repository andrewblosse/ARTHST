...
summary(basic_lm)
arrange(head(tidy(basic_lm), 23), p.value)
glance(basic_lm)
...
For the regression analysis of our data, we've begun with two primary models that reflect our research goals. Presently, we are using linear models - as our understanding of the data improves, we will use more appropriate types of GLMs that better reflect the non-linearity in the relationships between some of our variables. 

The first model aims to predict the inflation-adjusted price of a painting by its primary physical attributes -
country of origin, the subject & genre of the piece, and its size in square inches. As we expected, the predictive value of
solely visual characteristics and sizes is quite limited -  the R squared value of this model is .425, a value which indicates that approximately 42.5% of the variation in real prices is explained by linear relationships with our predictor variables. Although low for modelling purposes, 42.5% is quite high for a model that does not discriminate based on artist name. We expected all but the most famous paintings to have massively random pricing schemes, but the 42.5% figure suggests that general painting sales are less random than we might expect. 

Interpreting the model's coefficients begins with the nationality variable - the baseline dummy variable is "American" and thus each coefficient represents the difference between average paintings in a nationality and average American paintings. Unsurprisingly, the Dutch, Flemish, Bohemian, Greek & Spanish, Italian, and Russian coefficients are flagged as significant to at least the 1% level. All of these nationalities have positive coefficients and thus are more valuable on average than American works. However, "Unknown" nationality does not appear to be significant - one would expect that an unknown nationality would systematically lower its price, but the decrease observed in our data seems to be random noise.

`sale_date_year` is both statistically significant and has a positive coefficient of $2261 - a figure which suggests that each additional year leads to an average rise of $2261 in inflation-adjusted dollars. This figure is unsurprising - 

The second model more directly
addresses our variable of interest - price per square inch - and its relationship with the prior variables excluding raw size. 


```{r savingfunctions, eval=FALSE}
png(filename = "plots/years_average_prices_usd.png")
years_average_prices_usd
dev.off()
...
nongolden_means <- sample(golden_paintings$price_amount, size = c(10000, 1000), replace = TRUE)
